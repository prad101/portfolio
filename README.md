...
deploying under Kubernetes with auto-scaling. Achieved 75% in-house closure, resulting in monthly
savings of more than 7 thousand dollars
- Developed an image quality aware model for face recognition system on edge devices. The model reduced the
face missing errors by 90%
...
- Built comprehensive dashboards in Google Looker Studio with predictive modeling to track chatbot
performance,reduced average query response time by 80%
- Trained an NLP model to extract key information from multilingual text messages and categorized on various
...
**Conversational AI Support Assistant** (Part of OCI certification) | LangChain, OpenAI, Vector DB
- Developed a Langchain based chatbot using OpenAI to create a support assistant that answers technical queries of
a product by leveraging its documentation through RAG (FAISS).

**Simplify Research** (Academic Project) | OCR (pytesseract), regex, OpenAI
- Built an OCR-assisted preprocessing framework to segment and extract core research paper components, optimizing LLM-based summarization for higher accuracy, with reduced token usage, and improved speed. Applications: Academic summarization tools, Automated review systems.
